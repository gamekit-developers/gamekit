/*
bParse
Copyright (c) 2006-2009 Erwin Coumans  http://gamekit.googlecode.com
...
{
	mMemoryDNA = new bDNA();
	mMemoryDNA->init((char*)BulletDNAstr,BulletDNAlen);
}
...
void	btBulletFile::writeDNA(FILE* fp)
{
	...
	{
		dataChunk.len = BulletDNAlen;
		dataChunk.oldPtr = BulletDNAstr;
		fwrite(&dataChunk,sizeof(bChunkInd),1,fp);
		fwrite(BulletDNAstr, BulletDNAlen,1,fp);
	}
}
...
void	btBulletFile::parse(bool verboseDumpAllTypes)
{
	...
	{
		parseInternal(verboseDumpAllTypes,(char*)BulletDNAstr,BulletDNAlen);
	}
}
