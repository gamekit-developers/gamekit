...
typedef utArray<VertexBoneAssignment>                AssignmentList;
...
{
public:
    gkMeshLoaderPrivate(gkBlendFile *file, gkMeshLoader *parent, Blender::Object *ob,  Mesh *mesh);

    void convertMesh(void);
    void convertSkeleton(Skeleton *skel, AssignmentListMap &ret,  bool& index);
    void findWeight(int index, class gkSubMeshBufferItem *sub, SubMesh *me, AssignmentListMap &assign);


    Blender::Object *object;
    gkString skeletonName;
    Mesh *mesh;
...
gkMeshLoaderPrivate::gkMeshLoaderPrivate(gkBlendFile *f, gkMeshLoader *parent, Blender::Object *ob, Mesh *m) :
...
void gkMeshLoaderPrivate::convertMesh(void)
{
    ...
    if (!mface || !mvert)
    {
        gkPrintf("Skipping mesh '%s', it has no usable data "
            "faces,vertices [%i, %i]\n", bmesh->id.name + 2, bmesh->totface, bmesh->totvert);
        return;
    }

    if ((bmesh->totvert) > LIMIT)
    {
        gkPrintf("Maximum vertex limit exceeded. limit = %i\n", LIMIT);
        return;
    }
    ...
    {
        assignments.reserve(bmesh->totvert);
        SkeletonPtr skel = SkeletonManager::getSingleton().load(GKB_IDNAME(object->parent), mesh->getGroup());
        if (!skel.isNull())
            convertSkeleton(skel.getPointer(), assignments, hasAssignments);
    }
    ...
    bool openglVertexColor = Ogre::Root::getSingleton().getRenderSystem()->getName().find("OpenGL") != GK_NPOS;
    ...
    for (int fi = 0; fi < bmesh->totface; fi++)
    {
        ...
        if (isQuad)
        {
            ...
            if (vael < vbel)
            {
                ...
                if (hasAssignments)
                {
                    tri[0].addVertexBoneAssignment(0, curface.v1);
                    tri[0].addVertexBoneAssignment(1, curface.v2);
                    tri[0].addVertexBoneAssignment(2, curface.v3);

                    tri[1].addVertexBoneAssignment(0, curface.v3);
                    tri[1].addVertexBoneAssignment(1, curface.v4);
                    tri[1].addVertexBoneAssignment(2, curface.v1);
                }
            }
            ...
            else
            {
                ...
                if (hasAssignments)
                {
                    tri[0].addVertexBoneAssignment(0, curface.v1);
                    tri[0].addVertexBoneAssignment(1, curface.v2);
                    tri[0].addVertexBoneAssignment(2, curface.v4);

                    tri[1].addVertexBoneAssignment(0, curface.v4);
                    tri[1].addVertexBoneAssignment(1, curface.v2);
                    tri[1].addVertexBoneAssignment(2, curface.v3);
                }
            }
        }
    ...
    while (iter.hasMoreElements())
    {
        ...
        else
        {
            ...
            if (!useExternal)
            {
                ...
                gkString materialName = mesh->getName() + "/";
                materialName += gkString(me ? GKB_IDNAME(me) : "NULL") + "/";
            ...
            for (size_t cur = 0; cur < ele_len; ++cur)
            {
                ...
                for (;ele != eleend; ++ele)
                {
                    ...
                    else if (sem == VES_TEXTURE_COORDINATES)
                    {
                        ...
                        layer_count = gkClamp<int>(layer_count, TL_UV0, TL_UV7);
...
void gkMeshLoaderPrivate::convertSkeleton(Skeleton *skel, AssignmentListMap &ret, bool& hasAssignments)
{
    ...
    for (unsigned short b = 0; b < size; b++)
    {
        ...
        if (index != -1)
        {
            ...
            {
                for (int w = 0; w < dvert[n].totweight; w++)
                {
                    if (dvert[n].dw[w].def_nr == index)
                    {
                        Real weight = dvert[n].dw[w].weight;

                        hasAssignments = true;
                        VertexBoneAssignment vba;
                        vba.vertexIndex = 0; // assign after mesh is sorted / indexed
                        vba.boneIndex = bone->getHandle();
                        vba.weight = dvert[n].dw[w].weight;

                        size_t pos;
                        if ((pos = ret.find(n)) == GK_NPOS)
                        {
                            ret.insert(n, AssignmentList());
                            pos = ret.find(n);
                        }

                        if (pos != GK_NPOS)
                            ret.at(pos).push_back(vba);
                    }
                }
            }
        }
...
{
    if (sub->vbuf[index].vba_index == -1)
        return;

    size_t pos = assign.find(sub->vbuf[index].vba_index);
    if (pos != GK_NPOS)
    {
...
void gkMeshLoader::loadResource(Resource *resource)
{
    ...
    gkMeshLoaderPrivate converter(m_file,  this, m_object, static_cast<Mesh*>(resource));
