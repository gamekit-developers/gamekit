...
typedef Array<int>								AssignmentIndexList;
typedef Array<VertexBoneAssignment>				AssignmentList;
typedef HashTable<int, AssignmentList>			AssignmentListMap;

//-----------------------------------------------------------------------------
class MeshLoaderPrivate
{
public:
	MeshLoaderPrivate(BlendConverter *file, MeshLoader *parent, Blender::Object *ob,  Mesh *mesh);

	void convertMesh(void);
	void convertSkeleton(Skeleton *skel, AssignmentListMap &ret,  bool& index);
	void findWeight(int index, class SubMeshBufferItem *sub, SubMesh *me, AssignmentListMap &assign);
...
bool SubMeshSlot::blender_mat= false;


//-----------------------------------------------------------------------------
MeshLoaderPrivate::MeshLoaderPrivate(BlendConverter *f, MeshLoader *parent, Blender::Object *ob, Mesh *m) :
...
//-----------------------------------------------------------------------------
void MeshLoaderPrivate::convertMesh(void)
{
	...
	if (!mface || !mvert)
	{
		Logf("Skipping mesh %s, it has no usable data", bmesh->id.name);
		return;
	}

	if ((bmesh->totvert) > LIMIT)
	{
		Logf("Maximum vertex limit exceeded (%i)", LIMIT);
		return;
	}


	LoaderUtils ut(file->getInternalFile());
	...
	if (object->parent && object->parent->type == OB_ARMATURE)
	{
		...
		SkeletonPtr skel= SkeletonManager::getSingleton().load(OGRE_IDNAME(object->parent), mesh->getGroup());
		if (!skel.isNull())
	...
	bool openglVertexColor = Ogre::Root::getSingleton().getRenderSystem()->getName().find("OpenGL") != OGRE_NPOS;
	...
	for (int fi= 0; fi < bmesh->totface; fi++)
	{
		...
		FaceBufferObject tri[2]= {FaceBufferObject(), FaceBufferObject()};
		...
		if (foundItem == 0)
		{
			foundItem= new SubMeshBufferItem();
			...
			SlotItemPair p= {new SubMeshSlot(tester), foundItem};
	...
	while (iter.hasMoreElements())
	{
		const SubMeshSlot &slot= *(iter.peekNext().slot);
		SubMeshBufferItem *currentMesh= iter.peekNext().item;
		...
		if (count == 0)
		{
			const VertexBufferObject& vbo= currentMesh->vbuf[0];
		...
		if (SubMeshSlot::blender_mat)
		{
			...
			if (me != 0)
			{
				...
				if (ptr.isNull())
				{
					ptr= mgr.create(OGRE_IDNAME(me), mesh->getGroup());

					MaterialUtils util(file);
		...
		else
		{
...

			{
				...
				materialName += String(me ? OGRE_IDNAME(me) : "NULL") + "/";
		...
		/// build index items
		{
			...
			IndexBufferItem *ibuf= currentMesh->ibuf.ptr();

			for (size_t cur= 0; cur < ele_len; cur++)
			{
				const IndexBufferItem& i= ibuf[cur];
				if (using32)
				{
		...
		/// build vertex items
		{
			...
			VertexBufferObject *vbuf= currentMesh->vbuf.ptr();

			for (size_t cur= 0; cur < ele_len; ++cur)
			{
				const VertexBufferObject &vbo= vbuf[cur];
				int layer_count= 0;
				...
				for (;ele != eleend; ++ele)
				{
					...
					else if (sem == VES_TEXTURE_COORDINATES)
					{
						...
						layer_count= OgreClamp(layer_count, TL_UV0, TL_UV7);
...
//-----------------------------------------------------------------------------
void MeshLoaderPrivate::convertSkeleton(Skeleton *skel, AssignmentListMap &ret, bool& hasAssignments)
{
...
//-----------------------------------------------------------------------------
void MeshLoaderPrivate::findWeight(int index, SubMeshBufferItem *sub, SubMesh *me, AssignmentListMap &assign)
{
...
//-----------------------------------------------------------------------------
MeshLoader::MeshLoader(BlendConverter *fp, Blender::Object *ob) :
...
//-----------------------------------------------------------------------------
void MeshLoader::loadResource(Resource *resource)
{
	OGRE_ASSERT(m_file);
	OGRE_ASSERT(m_object);
	OGRE_ASSERT(resource);

	MeshLoaderPrivate converter(m_file,  this, m_object, static_cast<Mesh*>(resource));
