...
public:
	...
	void FileListFrameClosed() { m_fileListFrame = NULL; }
...
private:
	...
	void OnFileListWindow(wxCommandEvent& event);
...
public:
	...
	virtual ~SecondFrame()
	{
		EmbedFrame* parent = (EmbedFrame*)GetParent();
		if (parent)
			parent->SecondFrameClosed();
	}
};

class FileListFrame : public wxFrame
{
	wxListBox* m_listFile;
	wxDirPickerCtrl* m_dirPicker;
public:
	FileListFrame(wxWindow* parent)
		:	wxFrame(parent, wxID_ANY, "Blend FileList")
	{
		//generated by wxFormBuilder
		this->SetSizeHints( wxDefaultSize, wxDefaultSize, wxSize(WIN_SIZE_X/2,WIN_SIZE_Y));
	
		wxFlexGridSizer* fsize;
		fsize = new wxFlexGridSizer( 2, 1, 0, 0 );
		fsize->AddGrowableCol( 0 );
		fsize->AddGrowableRow( 1 );
		fsize->SetFlexibleDirection( wxBOTH );
		fsize->SetNonFlexibleGrowMode( wxFLEX_GROWMODE_SPECIFIED );
	
		m_dirPicker = new wxDirPickerCtrl( this, ID_FILELIST_PICKER, wxEmptyString, wxT("Select a folder"), wxDefaultPosition, wxDefaultSize, wxDIRP_DEFAULT_STYLE );
		fsize->Add( m_dirPicker, 0, wxALL|wxEXPAND, 5 );
	
		m_listFile = new wxListBox( this, ID_FILELIST_LIST, wxDefaultPosition, wxDefaultSize, 0, NULL, 0 ); 
		fsize->Add( m_listFile, 0, wxALL|wxEXPAND, 5 );
		...
		//--
		wxString dir = wxGetCwd() + wxFileName::GetPathSeparator();
		m_dirPicker->SetDirName(dir);
		OpenDirectory(dir);
	...
	virtual ~FileListFrame()
	{
		EmbedFrame* parent = (EmbedFrame*)GetParent();
		if (parent)
			parent->FileListFrameClosed();
	}

	void OpenDirectory(const wxString& location)
	{		
		if (!m_listFile)
			return;

		gkPrintf("Open directory: %s", (const char*)location);

		m_listFile->Clear();

		wxDir dir(location);
		wxString filename;
		wxString filespec("*.blend");
		...
		m_listFile->InsertItems(flist, 0);		
	}	

	void OnDirectoryChanged(wxFileDirPickerEvent& event)
	{
		OpenDirectory(event.GetPath());
	}

	void OnFileSelected(wxCommandEvent& event)
	{
		int i = m_listFile->GetSelection();
		if (i >=0)
		{		
			wxString fname = m_dirPicker->GetDirName().GetPath() + wxFileName::GetPathSeparator() + m_listFile->GetString(i);
			if (!wxFileName::FileExists(fname))
			{
				gkPrintf("File isn't exists. %s", (const char*)fname);
				return;
			}
			EmbedFrame* parent = (EmbedFrame*)GetParent();
			if (parent)
				parent->OpenBlendFile(fname);
		}
	}
...
BEGIN_EVENT_TABLE(FileListFrame, wxFrame)
	EVT_LISTBOX_DCLICK(ID_FILELIST_LIST, FileListFrame::OnFileSelected)
	EVT_DIRPICKER_CHANGED(ID_FILELIST_PICKER, FileListFrame::OnDirectoryChanged)
END_EVENT_TABLE()
...
BEGIN_EVENT_TABLE(EmbedFrame, wxFrame)
	...
	EVT_MENU(ID_FILELIST_WINDOW, EmbedFrame::OnFileListWindow)
		...
		m_fileListFrame(NULL)
{
	...
	menu->Append(ID_FILELIST_WINDOW, "&FileList Window");
...
void EmbedFrame::OnFileListWindow(wxCommandEvent& event)
{
	if (m_fileListFrame)
		return;

	m_fileListFrame = new FileListFrame(this);
	m_fileListFrame->SetPosition(GetScreenRect().GetTopRight());
}
