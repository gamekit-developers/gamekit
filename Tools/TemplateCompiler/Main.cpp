...
void Template_writeSource(FILE *fp, char *fileName, bool binary)
{
	...
	if (!buffer.empty())
    {
        ...
        Template_upper(base);
		if (binary)
			fprintf(fp, "const size_t %s_SIZE=%d;\n", base.c_str(), buffer.size());
        fprintf(fp, "static const %s char %s[]={\n", binary ? "unsigned" : "", base.c_str());
        fprintf(fp, "    ");
...
#endif
        Template_writeSource(fp, argv[i], binary);
    }
