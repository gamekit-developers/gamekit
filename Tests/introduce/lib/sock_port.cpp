...
//  If you get "file not found" for <netfw.h>, then you have two options:
//
//  1) Download and install the Windows XP SP2 Platform SDK from the MSDN 
//     site (msdn.microsoft.com/downloads), and point Visual Studio at 
//     this newly installed Platform SDK.
//
//  2) Change NEED_FIREWALL_ENABLE to 0 in the include file "sock_port.h".
//
//  The firewall enable feature makes the application request that it be 
//  added to the list of applications allowed to use networking in the 
//  Windows Firewall, new in XP SP2. Without this, networking applications 
//  may not work without disabling the Windows Firewall. (Note that the 
//  Windows Server 2003 version of the Platform SDK doesn't contain the 
//  right headers -- it's too old).
//
//  The Platform SDK is the official set of headers, libraries, and 
//  documentation for development for Windows. More info at msdn.microsoft.com
//
#include <netfw.h>
