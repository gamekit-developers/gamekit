...
//-----------------------------------------------------------------------------
LoaderUtils::LoaderUtils(bParse::bBlenderFile *file) :
		m_file(file)
{
	OGRE_ASSERT(m_file); // Required
}

//-----------------------------------------------------------------------------
void LoaderUtils::getLayers(Blender::Mesh *mesh, Blender::MTFace **eightLayerArray, Blender::MCol **oneMCol, int &validLayers)
{
	OGRE_ASSERT(mesh);
...
//-----------------------------------------------------------------------------
int LoaderUtils::getDeformGroupIndex(Blender::Object *ob, const char *group)
{
...
//-----------------------------------------------------------------------------
Blender::Material* LoaderUtils::getMaterial(Blender::Object *ob, int index)
{
	if (!ob || ob->totcol==0) return 0;

	index= OgreClamp(index, 1, ob->totcol);
	Blender::Material* ma;
