...
TextureLoader::TextureLoader(BlendConverter *fp, Blender::Image *ima) :
...
void TextureLoader::loadResource(Resource* resource)
{
	OGRE_ASSERT(m_file);
	OGRE_ASSERT(m_image);
	...
	{
		Logf("Warning: Skipping image %s no packed file information is present!", texture->getName().c_str());
		return;
	}
