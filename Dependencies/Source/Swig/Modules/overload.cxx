...
String *Swig_overload_dispatch_self(Node *n, const_String_or_char_ptr fmt, int *maxargs)
{
    int i, j;
    *maxargs = 1;
    ...
    for (i = 0; i < nfunc; i++)
    {
        ...
        if (!varargs)
        {
            if (num_required == num_arguments)
            {
                Printf(f, "if (%s == %d) {\n", argc_template_string, (num_required-1));
            }
            else
            {
                Printf(f, "if ((%s >= %d) && (%s <= %d)) {\n", argc_template_string, num_required, argc_template_string, num_arguments);
        ...
        while (pj)
        {
            ...
            if (j >= num_required)
            {
                String *lfmt = ReplaceFormat(fmt, num_arguments-1);
                Printf(f, "if (%s <= %d) {\n", argc_template_string, j);
                Printf(f, Char(lfmt), Getattr(ni, "wrap:name"));
                Printf(f, "}\n");
                Delete(lfmt);
