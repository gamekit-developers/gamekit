...
const Ogre::String gkParticleEmitter::NAME = "gkEmitter";

gkParticleEmitter::gkParticleEmitter(Ogre::ParticleSystem* psys)
...
gkParticleEmitter::~gkParticleEmitter()
{
}

unsigned short gkParticleEmitter::_getEmissionCount(Ogre::Real timeElapsed)
{
	return genConstantEmissionCount(timeElapsed);
}

void gkParticleEmitter::copyParametersTo(Ogre::StringInterface* dest) const
{
	Ogre::ParticleEmitter::copyParametersTo(dest);
	static_cast<gkParticleEmitter*>(dest)->m_creator = m_creator;
}
...
void gkParticleEmitter::_initParticle(Particle* pParticle)
{
	GK_ASSERT(m_creator);
	...
	if (mParent)
	{
		...
		if (!any.isEmpty())
		{
			gkParticleObject* pobj = Ogre::any_cast<gkParticleObject*>(any);
...
Ogre::ParticleEmitter* gkEmitterFactory::createEmitter(Ogre::ParticleSystem* psys) 
{
	gkParticleEmitter* emit = new gkParticleEmitter(psys);
