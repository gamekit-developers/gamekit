...
gkGameObjectChannel::~gkGameObjectChannel()
{
}
...
void gkGameObjectChannel::evaluate(gkScalar time, gkScalar delta, gkScalar weight)
{
	...
	if (weight < 1.f)
	{
		...
		channel.rot = gkMathUtils::interp(channel.rot, cur.rot, weight);
		channel.rot.normalise();
		channel.scl = gkMathUtils::interp(channel.scl, cur.scl, weight);

	}

	m_object->applyTransformState(channel);
}
